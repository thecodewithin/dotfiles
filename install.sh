...
#   3) Stow the tmux configuration directly to $HOME (sources: tmux/.tmux and tmux/.tmux.conf).
#   4) Stow the Neovim configuration directly to $HOME (source: nvim/.config/nvim).
...
# Step 2: Stow Vim configuration (defaults to $HOME)
echo "Stowing Vim configuration…"
stow vim

# Step 3: Stow tmux configuration (defaults to $HOME)
